  ...
  <header
    ...
    <div class="max-w-350 mx-auto px-6 flex items-center justify-between gap-6">
      ...
      <NuxtLink to="/" class="flex items-center gap-2 shrink-0">
        <div class="flex items-end">
          <div class="flex flex-col items-center">
            ...
            <span class="text-5xl font-black leading-none text-[#4a7c3f]" style="font-family: 'Arial Black', sans-serif;">P</span>
          </div>
          <div class="relative -ml-1 flex flex-col items-center">
            <div class="relative">
              ...
              <span class="relative text-5xl font-black leading-none text-[#4a7c3f] z-10" style="font-family: 'Arial Black', sans-serif;">S</span>
            </div>
          </div>
        </div>
      ...
      <div class="hidden lg:flex items-center gap-3 shrink-0">
        ...
        <div v-else class="flex items-center gap-4">
          <NuxtLink to="/profile" class="flex items-center gap-3 bg-[#f0f7f4] hover:bg-[#e4efeb] p-1.5 pr-5 rounded-full border border-[#4a7c3f]/10 transition-all group">
            <img 
    ...
    <div v-if="menuOpen" class="lg:hidden border-t border-gray-100 bg-white px-6 py-6 space-y-4">
        <div v-if="user" class="flex items-center gap-4 p-3 bg-gray-50 rounded-2xl">
            <img :src="user.avatar || 'https://i.pravatar.cc/100'" class="w-12 h-12 rounded-full border-2 border-[#4a7c3f]" />
            <div>
              <p class="font-black text-gray-900">{{ user.name }}</p>
              <NuxtLink @click="menuOpen = false" to="/profile" class="text-xs font-bold text-[#4a7c3f] underline">View Profile</NuxtLink>
            </div>
        </div>


        <nav class="flex flex-col gap-2">
           <NuxtLink v-for="link in navLinks" :key="link.label" :to="link.to" @click="menuOpen = false" class="py-2 font-bold text-gray-700 hover:text-blue-600">{{ link.label }}</NuxtLink>

        </nav>

        <div class="pt-4 border-t border-gray-50">
          <NuxtLink v-if="!user" to="/signup" @click="menuOpen = false" class="block text-center bg-blue-600 text-white py-3 rounded-xl font-bold">Sign Up Free</NuxtLink>
          <button v-else @click="logout" class="block w-full text-center bg-red-50 text-red-500 py-3 rounded-xl font-bold">Log Out</button>
        </div>
    </div>
  </header>
</template>
